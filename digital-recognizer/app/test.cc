...
int reduce_train_feature()
{
  ...
  debug_info("train_set_data.cols %d", train_set_data.cols);
  debug_info("train_set_data.rows %d", train_set_data.rows);
  ...
  train_feature      = train_set_data.colRange(1,train_set_cols);
	...
	//cv::Mat reduced_train_feature = pca_reduce_dimension(train_feature, 80);

  // write the result to file 
	//write_max2csv(reduced_train_feature,DATA_DIR"/pca_reduced80_train_feature.csv");
  return 0;
}
...
int main(int argc, const char * argv[])
{
  ...
  // test read csv file function
  debug_info("read data");

  //test_reduce_fun();
  //test_random_permutation_generator();
  test_knn_classifier_find_maxk();
  //test_knn_classifier();
